...
def K3_analysis(k_min, k_max, l_min, n_term_min, n_term_max, kc_init, y_init):
    ...
    for n_term in tqdm(range(n_term_min, n_term_max), desc = 'K3_g_fit_no_corrections'):
        ...
        popt, pcov = curve_fit(poly, (sorted_K, sorted_L), sorted_K3_g, p0 = initParam, sigma = sorted_err_K3_g, absolute_sigma = True, bounds = ((0.05, 1.2, 2.5, *inf_bounds), (0.1, 1.9, 5.0, *sup_bounds)))
    ...
    for omega in tqdm(np.arange(omega_min, omega_max, omega_step), desc = 'K3_g_fit_w_corrections'):
        for n_term1 in range(n_term_min, n_term_max):
            for n_term2 in range(n_term_min, n_term_max):
                ...
                popt, pcov = curve_fit(poly_corrections, (sorted_K, sorted_L), sorted_K3_g, p0 = initParam, sigma = sorted_err_K3_g, absolute_sigma = True, bounds = ((0.05, 1.40, 4.0, *inf_bounds), (0.10, 1.80, 7.0, *sup_bounds)))
...
def K2_analysis(k_min, k_max, l_min, n_term_min, n_term_max, kc_init, y_init):
    ...
    for n_term in tqdm(range(n_term_min, n_term_max), desc = 'K2_g_fit_no_corrections'):
        inf_bounds = -np.ones(n_term)/2.0
        sup_bounds = np.ones(n_term)/2.0
        popt, pcov = curve_fit(poly, (sorted_K,sorted_L), sorted_K2_g, p0 = initParam, sigma = sorted_err_K2_g, absolute_sigma = True, bounds = ((0.05, 1.40, 2.5, *inf_bounds), (0.1, 1.80, 4.0, *sup_bounds)))
...
# DEFINISCO LA FUNZIONE DI ANALISI CON CORREZIONI
def K2_analysis_corrections(k_min, k_max, l_min, n_term_min, n_term_max, omega_min, omega_max, omega_step, eps, kc_init, y_init):
    # def poly_corrections(X, a, b, d, *c):
    #     k,l = X
    #     poly = 0
    #     for i in range(n_term1):
    ...
    # n_term3 = (n_term_min + n_term_max)//2
    # c = np.zeros(n_term_min + n_term_min, dtype = 'float')
    c = np.zeros(3*n_term_min, dtype = 'float')
    # initParam = [0.07, 1.48, 1.48*2, *c]
    initParam = [kc_init, y_init, *c]
    for omega in tqdm(np.arange(omega_min, omega_max, omega_step), desc = 'K2_g_fit_w_corrections'):
        for n_term1 in range(n_term_min, n_term_max):
            for n_term2 in range(n_term_min, n_term_max):
                for n_term3 in range(n_term_min, n_term_max):

                    inf_bounds = -np.ones(n_term3 + n_term1 + n_term2)*50
                    sup_bounds = np.ones(n_term3 + n_term1 + n_term2)*50

                    # popt, pcov = curve_fit(poly_corrections, (sorted_K, sorted_L), sorted_K2_g, p0 = initParam, sigma = sorted_err_K2_g, absolute_sigma = True, bounds = ((0.05, 1.40, 2.5, *inf_bounds), (0.1, 1.80, 5.0, *sup_bounds)))
                    popt, pcov = curve_fit(poly_corrections, (sorted_K, sorted_L), sorted_K2_g, p0 = initParam, sigma = sorted_err_K2_g, absolute_sigma = True, bounds = ((0.05, 1.45, *inf_bounds), (0.1, 1.85, *sup_bounds)), max_nfev = 500*len(sorted_K)*len(sorted_K))
                ...
                c = np.zeros(n_term2 + n_term1 + n_term_min+1, dtype = 'float')
                # initParam = [0.07, 1.48, 1.48*2, *c]
                initParam = [kc_init, y_init, *c]

            c = np.zeros(n_term_min + n_term_min + n_term1 + 1, dtype = 'float')
            # initParam = [0.07, 1.48, 1.48*2, *c]
            initParam = [kc_init, y_init, *c]
    ...
    return plot_params, n_term1_K2_g, n_term2_K2_g, n_term3_K2_g, omega_plot, mean_Kc, std_Kc, mean_yk, std_yk, mean_chisq_red
