...
def K3_analysis(k_min, k_max, l_min, n_term_min, n_term_max):
    ...
    initParam = [0.07, 1.76, 3*1.76, *c]

    # CICLO SUL NUMERO DI TERMINI DEL POLINOMIO
    for n_term in tqdm(range(n_term_min, n_term_max), desc = 'K3_fit_no_corrections'):
        inf_bounds = -np.ones(n_term)*10
        sup_bounds = np.ones(n_term)*10
        popt, pcov = curve_fit(poly, (K,L), K3, p0 = initParam, sigma =err_K3, absolute_sigma = True, bounds = ((0.05, 1.2, 4.0, *inf_bounds), (0.1, 1.9, 10.0, *sup_bounds)))

        initParam = popt
        initParam = np.append(initParam, 0.0)

        chiq = np.sum(((K3 - poly((K,L), *popt)) / (err_K3))**2)
        ndof = len(K3) - len(initParam)
        red_chiq = chiq/ndof
    ...
    n_term_K3 = int(term[np.argmin(delta)])
    red_chisq_opt = red_chisq[np.argmin(delta)]

    return popt, err_opt, n_term_K3, red_chisq_opt

# DEFINISCO LA FUNZIONE DI ANALISI CON CORREZIONI
# PER ORA SOLO CORREZIONI CON SCALING IN -OMEGA
def K3_analysis_corrections(k_min, k_max, l_min, n_term_min, n_term_max, omega_min, omega_max, omega_step, eps):
    ...
    initParam = [0.07, 1.76, 1.76*3, *c]
    for omega in tqdm(np.arange(omega_min, omega_max, omega_step), desc = 'K3_fit_w_corrections'):
        for n_term1 in range(n_term_min, n_term_max):
            for n_term2 in range(n_term_min, n_term_max):
                ...
                popt, pcov = curve_fit(poly_corrections, (K,L), K3, p0 = initParam, sigma = err_K3, absolute_sigma = True, bounds = ((0.05, 1.0, 0.0, *inf_bounds), (0.10, 3, 10, *sup_bounds)))

                initParam = popt
                initParam = np.append(initParam, 0.0)

                chiq = np.sum(((K3 - poly_corrections((K,L), *popt)) / (err_K3))**2)
                ndof = len(K3) - len(initParam)
                red_chiq = chiq/ndof
    ...
    plot_params = [Kc[np.argmin(delta)], yk[np.argmin(delta)], theta3[np.argmin(delta)] *cc]
    n_term1_K3 = int(term1[np.argmin(delta)])
    n_term2_K3 = int(term2[np.argmin(delta)])
    omega_plot = omg[np.argmin(delta)]

    return plot_params, n_term1_K3, n_term2_K3, omega_plot, mean_Kc, std_Kc, mean_yk, std_yk, mean_theta3, std_theta3, mean_chisq_red

# DEFINISCO LA FUNZIONE DI ANALISI PER K4
def K2_analysis(k_min, k_max, l_min, n_term_min, n_term_max):
    # DEFINISCO LE FUNZIONI DI FIT
    def poly(X, a, b, d, *c):
        k,l = X
        poly = 0
    ...
    initParam = [0.07, 1.74, 1.74*4, *c]

    # CICLO SUL NUMERO DI TERMINI DEL POLINOMIO
    for n_term in tqdm(range(n_term_min, n_term_max), desc = 'K2_fit_no_corrections'):
        inf_bounds = -np.ones(n_term)
        sup_bounds = np.ones(n_term)
        popt, pcov = curve_fit(poly, (K,L), K2, p0 = initParam, sigma =err_K2, absolute_sigma = True, bounds = ((0.05, 1.74, 5.0, *inf_bounds), (0.1, 1.78, 7.0, *sup_bounds)))

        initParam = popt
        initParam = np.append(initParam, 0.5)

        chiq = np.sum(((K2 - poly((K,L), *popt)) / (err_K2))**2)
        ndof = len(K2) - len(initParam)
        red_chiq = chiq/ndof
        perr = np.sqrt(np.diag(pcov))
        Kc = np.append(Kc, popt[0])
        err_Kc = np.append(err_Kc, perr[0])
    ...
    n_term_K2 = int(term[np.argmin(delta)])
    red_chisq_opt = red_chisq[np.argmin(delta)]

    return popt, err_opt, n_term_K2, red_chisq_opt

# DEFINISCO LA FUNZIONE DI ANALISI CON CORREZIONI
def K2_analysis_corrections(k_min, k_max, l_min, n_term_min, n_term_max, omega_min, omega_max, omega_step, eps):
    def poly_corrections(X, a, b, d, *c):
        k,l = X
        poly = 0
        for i in range(n_term1):
    ...
    c = np.zeros(n_term_min+n_term_min, dtype = 'float')
    initParam = [0.07, 1.76, 1.76*3, *c]
    for omega in tqdm(np.arange(omega_min, omega_max, omega_step), desc = 'K2_fit_w_corrections'):
        for n_term1 in range(n_term_min, n_term_max):
            for n_term2 in range(n_term_min, n_term_max):
                inf_bounds = -np.ones(n_term1 + n_term2)
                sup_bounds = np.ones(n_term1 + n_term2)
                popt, pcov = curve_fit(poly_corrections, (K,L), K2, p0 = initParam, sigma = err_K2, absolute_sigma = True, bounds = ((0.05, 1.74, 5.0, *inf_bounds), (0.1, 1.78, 7.0, *sup_bounds)))

                initParam = popt
                initParam = np.append(initParam, 0.0)

                chiq = np.sum(((K2 - poly_corrections((K,L), *popt)) / (err_K2))**2)
                ndof = len(K2) - len(initParam)
                red_chiq = chiq/ndof
    ...
    # PARAMETRI PER PLOT, DA NON PRENDERE TROPPO SUL SERIO
    cc = np.array(coeff[np.argmin(delta)])
    plot_params = [Kc[np.argmin(delta)], yk[np.argmin(delta)], theta2[np.argmin(delta)] *cc]
    n_term1_K2 = int(term1[np.argmin(delta)])
    n_term2_K2 = int(term2[np.argmin(delta)])
    omega_plot = omg[np.argmin(delta)]

    return plot_params, n_term1_K2, n_term2_K2, omega_plot, mean_Kc, std_Kc, mean_yk, std_yk, mean_theta2, std_theta2, mean_chisq_red
