...
def FO_intersection(path_filenames, j_min, j_max, l_min, l_max, n_term_min, n_term_max, inter_min, inter_max, ctrl_plot):
    ...
    for fname in path_filenames:
        with open(fname) as infile:
            ...
            if((aux_L[0]>=l_min) & (aux_L[0]<=l_max)):
                ...
                p, red_chisq_opt_FO, n_term_opt = FO_fit(sorted_J, sorted_corr_len/sorted_L, sorted_err_corr_len/sorted_L, j_min, j_max, n_term1, n_term2)
                coeffs.append(p)
                ...
                if(ctrl_plot == True):
                    ...
                    x = np.linspace(sorted_J.min(), sorted_J.max(), 10000)
                    # pl.errorbar(sorted_J, sorted_U, sorted_err_U, ls = '', fillstyle = 'none', marker = 'o', label = 'L='+str(int(aux_L[0])))
                    pl.errorbar(sorted_J, sorted_corr_len/sorted_L, sorted_err_corr_len/sorted_L, ls = '', fillstyle = 'none', marker = 'o', label = 'L='+str(int(aux_L[0])))
                    pl.plot(x, curve(x))
                    pl.xlim(j_min, j_max)
                    pl.legend()
    ...
    return mean_intersect, err_intersect, n_term_opt, red_chisq_opt_FO
